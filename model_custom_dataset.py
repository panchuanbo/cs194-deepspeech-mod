...
class DeepSpeech(nn.Module):
    ...
    #CHANGED A LOT OF THIS FOR TESTING
    def forward(self, x, lengths):
        #print("X at beginning", x.shape)
        ...
        #print("X after collapsing feature dimension", x.shape)
        x = x.transpose(1, 2).transpose(0,1).contiguous()  # TxNxH
        #print("X after first transpose", x.transpose(1, 2).contiguous().shape)
        #print("X after second transpose", x.shape)
        ...
        #print("X after lookahead", x.shape)

        x = self.fc(x)
        #print("Shape after the fully connected layer: ", x.shape)
        x = x.transpose(0, 1)
        
        #print("Shape after the transpose: ", x.shape)
        # identity in training mode, softmax in eval mode
        #print("First element before softmax: ", x[0, 0, :]/sum(x[0, 0, :]))
        
        #sums = torch.sum(x, dim=-1).unsqueeze(dim=-1)
        #x = x / sums
        #print("Sum along last dimension", torch.sum(x, dim=-1))
        
        #agex = x[:, :, :8]
        #print("Shape after the transpose age: ", agex.shape)
        #genderx = x[:, :, 8:10]
        #print("Shape after the transpose gender: ", genderx.shape)
        #accentx = x[:, :, 10:]
        #print("Shape after the transpose accent: ", accentx.shape)
        #age_softmax, isValidation = self.inference_softmax(agex)
        #gender_softmax = self.inference_softmax(genderx)
        #accent_softmax = self.inference_softmax(accentx)
        #x = torch.cat((agex, genderx, accentx), dim=-1)
        #print("Shape after the identity (in train) or softmax (in eval): ", x.shape)
        #print("Analyze this shape and run three softmaxes instead of 1")
        #print("Output lengths: ", output_lengths)
        
        x = x.mean(1)
        #if (isValidation):
        #    print("X after softmax", x[0, 0, :])
        return x
    ...
    def serialize(model, optimizer=None, epoch=None, iteration=None, loss_results=None,
                  age_results=None, gender_results=None, accent_results=None, avg_loss=None, meta=None):
        package = {
        ...
        if loss_results is not None:
            ...
            package['age_results'] = age_results
            package['gender_results'] = gender_results
            package['accent_results'] = accent_results
        if meta is not None:
            package['meta'] = meta
        return package
...
if __name__ == '__main__':
    ...
    if package.get('loss_results', None) is not None:
        ...
        print("  Current Age Ac:    {0:.3f}".format(package['age_results'][epochs - 1]))
        print("  Current Gender Ac: {0:.3f}".format(package['gender_results'][epochs - 1]))
        print("  Current Accent Ac: {0:.3f}".format(package['accent_results'][epochs - 1]))
